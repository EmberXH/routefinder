...
class DVRPTW_NR_TJ_Dataset(Dataset):
    CUST_FEAT_SIZE = 7
    ...
    def generate(cls,
        ...
        is_dyn_nr = torch.empty(size).bernoulli_(dod)
        ...
        tj_time = is_dyn_tj * is_tj_e * torch.randint(1, horizon // 3 + 1, dist_size, dtype=torch.float) \
                  + is_dyn_tj * (1 - is_tj_e) * torch.randint(horizon // 3 + 1, 2 * horizon // 3 + 1, dist_size,
                                                              dtype=torch.float)
        nr_aprs = is_dyn_nr * is_dyn_e * torch.randint(1, horizon // 3 + 1, size, dtype=torch.float) \
               + is_dyn_nr * (1 - is_dyn_e) * torch.randint(horizon // 3 + 1, 2 * horizon // 3 + 1, size,
                                                           dtype=torch.float)
        ...
        rdys = has_tw * (nr_aprs + torch.rand(size) * (horizon - torch.max(tts + durs, tws) - nr_aprs))
        rdys.floor_()
        extra = torch.zeros_like(durs)
        # Regroup all features in one tensor 坐标，需求，时间窗，服务时间
        customers = torch.cat((locs[:, 1:], dems, rdys, rdys + tws, durs, nr_aprs), 2)
        # 得到每两个点的距离
