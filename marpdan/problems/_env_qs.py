...
class DVRPTW_QS_Environment:
    CUST_FEAT_SIZE = 7
    VEH_STATE_SIZE = 4

    def __init__(self, data, nodes=None, cust_mask=None, changed_dem=None,
    ...
    def reset(self):
        ...
        self.cust_mask = torch.zeros_like(self.changed_dem).bool()
        ...
        # self.nodes_seq_cache = []
        # self.nodes_seq_cache.append(self.nodes)
    ...
    def _update_mask(self, cust_idx):
        ...
        #                                                 self.cur_veh[:, :, None, 2] - self.nodes_seq_cache[-1][:, None,
        #                                                                               :, 2] < 0)
        overload = torch.zeros_like(self.mask).scatter_(1,
                                                        self.cur_veh_idx[:, :, None].expand(-1, -1, self.nodes_count),
    ...
    def step(self, cust_idx):
        # dest = self.nodes_seq_cache[-1].gather(1, cust_idx[:, :, None].expand(-1, -1, self.CUST_FEAT_SIZE))
