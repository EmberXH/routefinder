...
class DVRPTW_VB_Environment:
    CUST_FEAT_SIZE = 6
    VEH_STATE_SIZE = 4
    def __init__(self, data, nodes=None, cust_mask=None, b_idx=None, b_time=None,
    ...
    def _update_broken(self):  # 处理损坏车辆
        broken_happen = torch.zeros(self.minibatch_size, dtype=torch.bool)
        for i in range(self.minibatch_size):
            if self.cur_veh_idx[i] == torch.tensor(self.b_veh_idx):
                # 发生损坏
                if torch.squeeze(self.cur_veh[:, :, 3]) > self.b_time[-1]:
                    broken_happen[i] = True
                    # 将损坏的车辆mask
                    self.b_veh_mask[i][self.b_veh_idx] = True
                    self.veh_done[i][self.b_veh_idx] = True
                    # 将损坏车辆的计划客户解除mask
                    self.mask[i, :, self.cur_cust_idx[i]] = False
                    self.cur_veh_mask[i, :, self.cur_cust_idx[i]] = False
                    self.done = bool(self.veh_done.all())
                    self.mask = self.mask | self.veh_done[:, :, None]
                    self.mask[:, :, 0] = 0
                    self._update_cur_veh()
        return broken_happen
    ...
    def step(self, cust_idx):
        ...
        if not self.is_ortools:
            ...
            # for i in range(self.minibatch_size):
            #     if not broken_happen[i]:
            #         continue
            #     # 将损坏的车辆mask
            #     self.b_veh_mask[i][self.b_veh_idx[i]] = True
            #     self.veh_done[i][self.b_veh_idx[i]] = True
            #     # 将损坏车辆的计划客户解除mask
            #     self.mask[i, :, self.cur_cust_idx[i]] = False
            #     self.served[i, self.cur_cust_idx[i]] = False
            self.done = bool(self.veh_done.all())
            self.mask = self.mask | self.veh_done[:, :, None]
            self.mask[:, :, 0] = 0
        self._update_cur_veh()
