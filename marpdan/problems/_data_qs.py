...
class DVRPTW_QS_Dataset(Dataset):
    CUST_FEAT_SIZE = 7

    def __init__(self, veh_count, veh_capa, veh_speed, nodes, cust_mask=None, changed_dem=None):
    ...
    def generate(cls,
        ...
        dataset = cls(veh_count, veh_capa, veh_speed, nodes, cust_mask, changed_dem)
        return dataset
    ...
    def __getitem__(self, i):
        if self.cust_mask is None:
            return self.nodes[i], self.changed_dem[i]
        else:
            return self.nodes[i], self.cust_mask[i], self.changed_dem[i]
