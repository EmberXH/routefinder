...
class DVRPTW_QC_Environment:
    ...
    def _update_hidden(self):  # 处理取消客户
        time = self.cur_veh[:, :, 3].clone()
        if self.init_cust_mask is None:
            cancel = ~self.served & (self.nodes[:, :, 6] <= time) & (
                        self.nodes[:, :, 6] != 0.0)  # 车辆时间大于取消时间，并且取消时间不能为0（不取消）
        else:
            cancel = ~(self.init_cust_mask | self.served) & (self.nodes[:, :, 6] < time) & (
                    self.nodes[:, :, 6] != 0.0)
        if cancel.any():
            # qc_main
            self.cust_mask = self.cust_mask | cancel
            self.mask = self.mask | cancel[:, None, :].expand(-1, self.veh_count, -1)
            self.served = self.served | cancel
